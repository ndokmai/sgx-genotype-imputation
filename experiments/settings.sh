...
N_TIMES=1 # average timing results over n runs
...
RUST_SMAC_DIR=$PROJECT_DIR/smac
RUST_SP_DIR=$PROJECT_DIR/service-provider
SP=$RUST_SP_DIR/target/release/smac-service-provider
HOST=$RUST_SMAC_DIR/target/release/host
CLIENT=$RUST_SMAC_DIR/target/release/client
...
STACK_SIZE=0x200000
N_THREADS=8
SGX_SP=$RUST_SP_DIR/target/x86_64-fortanix-unknown-sgx/release/smac-service-provider
ELF2SGXS="ftxsgx-elf2sgxs --heap-size $HEAP_SIZE --stack-size $STACK_SIZE --threads $N_THREADS $SGX_SP"
SGX_RUN_SP="ftxsgx-runner $SGX_SP.sgxs"
