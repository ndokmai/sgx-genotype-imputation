...
fn log_fwd_wind(mg: &[Symbol], params: &Params, num_refs: usize, 
                win_start: usize, win_size: usize) -> Array2<f64>
{
    ...
    s.slice_mut(s![0, ..]).assign(
        &(init.map(|x| x.ln()) + 
          emit.slice(s![k, .., win_start]).map(|x| x.ln())));
...
fn log_bwd_wind(mg: &[Symbol], params: &Params, num_refs: usize, 
                win_start: usize, win_size: usize) -> Array2<f64>
{
...
fn main()
{
    ...
    if args.len() != 3
    {
        eprintln!("Usage: {}\t<Reference_Haplotypes>\t<Imputation_Samples>", args[0]);
        return;
    }

    let ref_path = env::args().nth(1).unwrap();
    let ref_file = BufReader::new(
        File::open(&ref_path).expect("Cannot open reference haplotypes file"));

    let sample_path = env::args().nth(2).unwrap();
    let sample_file = BufReader::new(
        File::open(&sample_path).expect("Cannot open imputation samples file"));

    // It's safer to parse early to make sure the inputs aren't malformed
    let refs = ref_file.lines()
        .map(|l| 
             l.unwrap().chars()
             .map(|c| Symbol::parse(&c).unwrap()).collect::<Vec<Symbol>>())
        .collect::<Vec<_>>();
    ...
    {
        //println!("{}", n);
        let log_fw = log_fwd_wind(&mg[..], &params, 
                                  refs.len(), n * win_size, win_size);
        let log_bw = log_bwd_wind(&mg[..], &params, 
                                  refs.len(), n * win_size, win_size);
        ...
        for i in 0..win_size
        {
            ...
            for k in 0..4
            {
                let ans = &log_fb.slice(s![i, ..]) + 
                    &params.emit.slice(s![k, .., (n * win_size) + i]).map(|x| x.ln());
                let ans_max = ans.iter()
