...
fn usage(name: &str) {
    println!(
        "Usage:\n{} <path_to_sgxs_file> <path_to_ref_panels_file>",
        name
    );
}
...
fn parse_args() -> Result<(String, String), ()> {
    ...
    match args.len() {
        3 => Ok((args[1].to_owned(), args[2].to_owned())),
        _ => {
            usage(&args[0]);
            Err(())
        }
...
fn main() {
    ...
    let mut refs_file = File::open(&refs_file_path).expect("Cannot open reference panels file.");

    let mut child = if cfg!(feature = "simulation") {
        Command::new(&sgx_file_path).spawn().unwrap()
    } else {
